...
if(isset($_POST["rm_item_btn"])){
	$_SESSION['to_remove'] = $_POST["rm_item_btn"];
	header("Location: remove-item.php");
}
if(isset($_POST["ed_item_btn"])){
	$_SESSION['to_edit'] = $_POST["ed_item_btn"];
...
if(isset($_POST["submit_btn"])){
	//print_r($_POST);
	$sql_query = "SELECT title, category, price, description, contact, user.hash as uhash, item.hash as phash, login as added_by, photo1, photo2, photo3 ";
	$sql_query .= "FROM website.item, website.user ";
	$sql_query .= "where user.iduser = item.added_by ";
	$first = 1;
	if($_POST["Clothes"]) 
	{
		if($first == 1)
		...
		<div class="col-3  ">
			...
			<form method="post">
				...
				<div class="custom-control custom-checkbox">
					<input name="Clothes" type="checkbox" class="custom-control-input" <?php if($_POST['Clothes'] != NULL) echo checked ?> id="clothes">
					<label class="custom-control-label" for="clothes">Clothes</label>
				</div>
				<div class="custom-control custom-checkbox">
					<input name="Book" type="checkbox" class="custom-control-input"  <?php if($_POST['Book'] != NULL) echo checked ?> id="book">
					<label class="custom-control-label" for="book">Book</label>
				</div>
				<div class="custom-control custom-checkbox">
					<input name="Households" type="checkbox" class="custom-control-input" <?php if($_POST['Households'] != NULL) echo checked ?> id="Households">
					<label class="custom-control-label" for="Households">Households</label>
				</div>
				<div class="custom-control custom-checkbox">
		...
		<?php while($data=mysqli_fetch_assoc($run_query)){;?>
			...
			<div class="box border-bottom">
				...
				<div class="row text-centre">
					<div class="col ">
						<a  href="http://localhost/item.php?hash=<?php echo $data['phash']; ?>"><h3 ><?php echo $data['title']; ?> </h3></a>
					</div>
				</div>
				<div class="row ">
				<div class="col ">
				...
				<div class="row h30">
					...
					<div class="col-2 offset-4"><form method="post">
							<button name="rm_item_btn" value="<?php echo $data['phash'] ?>" class="btn but-pad vertical-center sunny-morning-gradient" type="submit">Remove</button>
						</form></div>
						<div class="col-2 "><form method="post">
							<button name="ed_item_btn" value="<?php echo $data['phash'] ?>" class="btn but-pad vertical-center sunny-morning-gradient" type="submit">Edit</button>
						</form></div>
				</div><?php } ?>
		</div> <?php } ?>
		</div>
