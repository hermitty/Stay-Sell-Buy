...
//UPDATE `website`.`item` SET `title` = 'ddd', `category` = 'ss', `description` = 'fghddjk', `price` = 'ee', `contact` = 'fghjeek' WHERE (`idvacancy` = '23');
require 'connect.php';
$phash = $_SESSION['to_edit'];
$sql_query = "SELECT * FROM website.item where item.hash = '$phash';";
...
if(isset($_POST["submit_btn"])){
	...
	if($select != 'Category' && !empty($title) && !empty($description) && !empty($price) && !empty($contact))
	{
		...
		$sql_query = "UPDATE `website`.`item` SET `title` = '$title', `category` = '$select', `description` = '$description', `price` = '$price', `contact` = '$contact' WHERE (`hash` = '$phash');";
