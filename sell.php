...
{
	$_SESSION['url'] = 'sell.php'; // <- change in other page
...
if(isset($_POST["submit_btn"])){
	...
	{
		move_uploaded_file($_FILES['file1']['tmp_name'], "img/item/{$_FILES['file1']['name']}");
		$image1 = "img/item/";
		$image1 .= "{$_FILES['file1']['name']}";
	}
	if($_FILES["file2"]["name"] != '')
	{
		move_uploaded_file($_FILES['file2']['tmp_name'], "img/item/{$_FILES['file2']['name']}");
		$image2 = "img/item/";
		$image2 .= "{$_FILES['file2']['name']}";
	}
	if($_FILES["file3"]["name"] != '')
	{
		move_uploaded_file($_FILES['file3']['tmp_name'], "img/item/{$_FILES['file3']['name']}");
		$image3 = "img/item/";
	...
	if($select != 'Category' && !empty($title) && !empty($description) && !empty($price) && !empty($contact))
	{
		...
		//instert to database
		$id = $_SESSION['id'];
		$sql_query = "INSERT INTO `website`.`item` (`title`, `category`, `description`, `price`, `contact`, `added_by`, `date`, `status`, `hash`, `photo1`, `photo2`, `photo3`)";
 ...
 <div class="animated fadeIn">
  ...
  <div class="main_buttons">
	...
	<form method="post" enctype="multipart/form-data" class="text-center border border-light p-4 centre" style="width:43%">
		...
		<!-- Title -->
		<div><input type="text" name="Title" id="AdditemTitle" class="form-control inp" placeholder="Title" value="<?php echo $title ?>"></div>
		...
		<div class=" inp">
			<textarea class="form-control rounded-1" name="Desription" id="AdditemDesription" rows="5" placeholder="Description"><?php echo $description ?></textarea>
		</div>
		...
		<!-- Price -->
		<div><input type="text" name="Price" id="AdditemPrice" class="form-control inp" placeholder="Price" value="<?php echo $price ?>"></div>
		...
		<!-- Contact -->
		<div><input type="text inp" name="Contact" id="AdditemContact" class="form-control inp" placeholder="Contact" value="<?php echo $contact ?>"></div>
